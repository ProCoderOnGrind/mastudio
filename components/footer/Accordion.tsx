...
export default function Accordion({ title, titleField, children }: { title: string; titleField?: string; children: React.ReactNode }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="border-t border-hairline py-3">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between">
        <span className="text-[15px]" data-tina-field={titleField}>{title}</span>
